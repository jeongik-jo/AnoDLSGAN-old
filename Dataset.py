...
def load_dataset(i_start, i_end):
    dataset = tfds.load(hp.train_dataset)
...
def load_ood_datasets(i_start, i_end):
    in_images, _ = _to_array(tfds.load(hp.train_dataset))
    ood_datasets = []
    ood_samples = []
    for dataset_name in hp.ood_datasets:
        ood_images, _ = _to_array(tfds.load(dataset_name))
        ood_images = ood_images * hp.ood_intensity + in_images * (1.0 - hp.ood_intensity)
        ood_dataset = tf.data.Dataset.from_tensor_slices({'image': ood_images[i_start: i_end]}).batch(
            hp.batch_size).prefetch(tf.data.AUTOTUNE)

        ood_datasets.append(ood_dataset)
        ood_samples.append(np.vstack(ood_images[:hp.save_image_size]))
